...
void RicEclRunnerDialog::setupUi()
{
    ...
    {
        m_triggerCombo->setFixedHeight( standardButtonHeight );
        m_triggerLabel->setFixedHeight( standardButtonHeight );
    }
    m_triggerCombo->setSizePolicy( QSizePolicy::Preferred, QSizePolicy::Preferred );
    processLayout->addWidget( m_triggerCombo );

    topLayout->addLayout( processLayout );
    // vertically center the process group relative to sibling widgets (e.g. the Add Files button)
    topLayout->setAlignment( processLayout, Qt::AlignVCenter );
    ...
    topLayout->setAlignment( m_addButton, Qt::AlignVCenter );
...
void RicEclRunnerDialog::slotAddFile()
{
    ...
    QStringList files =
        QFileDialog::getOpenFileNames( this, tr( "Select .data files" ), QString(), tr( "Data files (*.data);;All files (*.*)" ) );
...
void RicEclRunnerDialog::slotProcessFinished( int exitCode, QProcess::ExitStatus status )
{
    ...
    qDebug() << QDateTime::currentDateTime().toString( "hh:mm:ss.zzz" ) << "Process finished signal for row" << row << "exit" << exitCode
             << "status" << status;
